...
ChunkManager::ChunkManager()
{
	...
	int x = i / WORLD_SIZE;
	int z = i % WORLD_SIZE;
	WorldChunk* chunk = new WorldChunk(x * 32, z * 32);
...
bool ChunkManager::checkNeighbour(glm::vec3 position, glm::vec3 direction) {
    ...
    glm::vec3 internal_coord = glm::vec3((int)cube_coord.x % 32, cube_coord.y, (int)cube_coord.z % 32);
    auto chunk = m_chunks[WORLD_SIZE * (int)chunk_coord.x + (int)chunk_coord.z];
